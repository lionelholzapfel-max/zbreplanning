...
describe('Scoring Engine', () => {
  ...
  describe('Knockout Phase Scoring', () => {
    // Knockout matches use the same scoring as group stage
    // Score is based on fullTime (includes extra time if played)
    // No qualifier bonus - prediction is only on the score

    it('scores knockout match same as group stage', () => {
    ...
    it('handles draw in knockout (match went to penalties)', () => {
      // Match ended 1-1 after extra time, decided by penalties
      // We score based on the 1-1 fullTime score
      const predictions: Prediction[] = [
        { user_id: '1', home_score: 1, away_score: 1 },
        { user_id: '2', home_score: 0, away_score: 0 },
      ];
      const result: MatchResult = {
        home_score: 1,
        away_score: 1,
      ...
      const pointsMap = calculateMatchPoints(predictions, result, 'A', 'B');

      expect(pointsMap.get('1')?.base).toBe(3); // exact score
      expect(pointsMap.get('1')?.visionary).toBe(1); // solo exact
      expect(pointsMap.get('1')?.total).toBe(4);

      expect(pointsMap.get('2')?.base).toBe(2); // correct outcome (draw) + correct diff (0)
      expect(pointsMap.get('2')?.total).toBe(2);
    });
  });
});
